...
function formatCopay(copay: number | 'deductible'): string {
  return copay === 'deductible' ? 'After Ded.' : formatCurrency(copay);
}
...
export function exportComparisonToPDF(
  ...
  const scenarioText = [
    ...
    `Tier 1 Rx: ${scenario.tier1RxCount} | Tier 2 Rx: ${scenario.tier2RxCount} | Tier 3 Rx: ${scenario.tier3RxCount}`,
  ...
  const tableBody = [
    ...
    ['Coinsurance', ...plans.map((p) => `${100 - p.coinsurance}% / ${p.coinsurance}%`)],
...
export function exportPlanDetailToPDF(
  ...
  const detailsData = [
    ...
    ['Coinsurance', `${100 - plan.coinsurance}% / ${plan.coinsurance}%`],
    ...
    ['Tier 1 (Generic) Rx', formatCurrency(plan.rxTiers.tier1)],
    ['Tier 2 (Preferred) Rx', formatCurrency(plan.rxTiers.tier2)],
    ['Tier 3 Rx', formatCurrency(plan.rxTiers.tier3)],
